...
# INTEGRACIÓN ADICIONAL DE INFRAESTRUCTURA (SUPABASE)
...
# INICIALIZACIÓN SECTORIZADA DEL CLIENTE SUPABASE (CAPA DE PERSISTENCIA DISCRETA)
@st.cache_resource
...
def renderizar_bloque_chat_estructurado(afiliacion_id, nombre_cliente):
    ...
    if st.button("Enviar Mensaje al Historial", key=f"btn_send_{afiliacion_id}") and msg_input.strip() != "":
        # INSERCIÓN ADICIONAL EN SUPABASE (CANAL DE CHATS PERSISTENTE)
        if supabase:
            try:
                supabase.table("chats").insert({
                    "solicitud_id": afiliacion_id, "emisor": st.session_state["perfil"],
                    "motivo": motivo_sel, "msg": msg_input, "timestamp": datetime.now(timezone.utc).isoformat()
                }).execute()
            except Exception:
                pass
                ...
                if len(bloqueados) > 0:
                    ...
                    if st.button("Procesar Carga Definitiva (Inyección Real)"):
                        for _, r in df_editado.iterrows():
                            ...
                            # INSERCIÓN EN SUPABASE (MIGRACIÓN LOTE MASIVO)
                            if supabase:
                                try:
                                    supabase.table("solicitudes").insert(payload_masivo).execute()
                                except Exception:
                                    pass
                                    
                            st.session_state["base_datos_central"].append(payload_masivo)
                ...
                if st.form_submit_button("Inyectar Solicitud Manual"):
                    ...
                    if not re.match(r'^[JGEVVD]-[0-9]{8}-[0-9]$', rif_final):
                        ...
                        if dups:
                            ...
                            # INSERCIÓN EN SUPABASE (INGRESO MANUAL)
                            if supabase:
                                try:
                                    supabase.table("solicitudes").insert(payload_manual).execute()
                                except Exception:
                                    pass
                                    
                            st.session_state["base_datos_central"].append(payload_manual)
                    ...
                    if st.form_submit_button("🔄 Re-inyectar Registro a Bandeja Técnica y Reiniciar SLA"):
                        ...
                        if len(cta_f) < 10:
                            ...
                            if dups:
                                ...
                                # ACTUALIZACIÓN EN SUPABASE (APLICAR SUBSANACIÓN Y REINICIAR TIEMPO)
                                if supabase:
                                    try:
                                        supabase.table("solicitudes").update({
                                            "numero_cta": cta_f, "estatus": "1. Pendiente",
                                            "fecha_recibido": datetime.now(timezone.utc).isoformat(), "observaciones": obs_concat
                    ...
                    with st.expander(f"{p['SLA_Visual']} Empresa: {p['nombre_empresa']} | RIF: {p['rif']} | [{p['estatus']}] | Plazo: {p['SLA_Tiempo']}"):
                        if p["estatus"] == "1. Pendiente":
                            if st.button("👁️ Tomar Caso (Mudar a En Revisión)", key=f"btn_blq_{p['id']}"):
                                # ACTUALIZACIÓN EN SUPABASE (TOMAR CASO)
                                if supabase:
                                    try:
                                        supabase.table("solicitudes").update({"estatus": "2. En Revisión"}).eq("id", p["id"]).execute()
                                    except Exception:
                                        pass
                            ...
                            with col_a:
                                ...
                                if st.button("Estampar Cierre Técnico Definitivo", key=f"btn_ap_{p['id']}"):
                                    if wh_manual.strip() == "":
                                        ...
                                        # ACTUALIZACIÓN EN SUPABASE (APROBACIÓN TÉCNICA DEFINITIVA)
                                        if supabase:
                                            try:
                                                supabase.table("solicitudes").update({
                                                    "estatus": "4. Afiliado (Espera de Acompañamiento)", "wh": wh_manual,
                            ...
                            with col_b:
                                ...
                                if st.button("Confirmar Devolución F FIFO", key=f"btn_re_{p['id']}"):
                                    ...
                                    # ACTUALIZACIÓN EN SUPABASE (RECHAZO / DEVOLUCIÓN DE EXPEDIENTE)
                                    if supabase:
                                        try:
                                            supabase.table("solicitudes").update({
        ...
        if file_admin:
            ...
            if st.button("⚙️ Inyectar Repositorio y Ejecutar Algoritmo de Credenciales"):
                ...
                for _, r in df_editado_adm.iterrows():
                    ...
                    # INSERCIÓN EN SUPABASE (INYECCIÓN DE ALIMENTACIÓN ADMINISTRATIVA)
                    if supabase:
                        try:
                            supabase.table("solicitudes").insert(payload_admin).execute()
                        except Exception:
                            pass
                            
                    st.session_state["base_datos_central"].append(payload_admin)
            ...
            if not df_por_c.empty:
                ...
                if st.button("Forzar Reclasificación e Inyección"):
                    if nuevo_e == "4. Afiliado (Espera de Acompañamiento)" and wh_hist.strip() == "":
                        ...
                        # ACTUALIZACIÓN EN SUPABASE (RECLASIFICACIÓN DE HISTÓRICOS)
                        if supabase:
                            try:
                                supabase.table("solicitudes").update({
                                    "estatus": nuevo_e, "wh": wh_hist if wh_hist.strip() != "" else "None",
                                    "observaciones": f"[Saneamiento]: Migrado manualmente de 'Por Clasificar' a '{nuevo_e}'"
